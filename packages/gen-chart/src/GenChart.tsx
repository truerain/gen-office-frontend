...
type ResolvedMotion = {
  ...
  durationMs: number;
  easing: NonNullable<GenChartMotionOptions["easing"]>;
...
function resolveLegend(legend: boolean | GenChartLegendOptions | undefined) {
  ...
  if (legend === true)
    return { enabled: true, position: "top" as const, align: "start" as const };
  if (!legend)
    return {
      enabled: false,
      position: "top" as const,
...
function resolveTooltip<T>(
  tooltip: boolean | GenChartTooltipOptions<T> | undefined,
) {
...
function resolveMotion(
  motion: boolean | GenChartMotionOptions | undefined,
  tokens: ReturnType<typeof resolveChartTokens>,
): ResolvedMotion {
  if (motion === false) {
    return {
      enabled: false,
      mode: "none",
      durationMs: tokens.motion.durationMs,
      easing: tokens.motion.easing,
    };
  }
  if (motion === true || !motion) {
    return {
      enabled: tokens.motion.enabled,
      mode: tokens.motion.enabled ? "enter" : "none",
      durationMs: tokens.motion.durationMs,
      easing: tokens.motion.easing,
    };
  }
  const enabled = motion.enabled ?? tokens.motion.enabled;
  return {
    enabled,
    mode: enabled ? (motion.mode ?? "enter") : "none",
    durationMs: motion.durationMs ?? tokens.motion.durationMs,
    easing: motion.easing ?? tokens.motion.easing,
    changeKey: motion.changeKey,
  };
}

function motionEase(easing: ResolvedMotion["easing"]) {
  if (easing === "linear") return "linear";
  if (easing === "easeInOut") return "ease-in-out";
  return "cubic-bezier(0.16, 1, 0.3, 1)";
}

function useMotionRenderKey(
  ...
  React.useEffect(() => {
    if (
      !motion.enabled ||
      motion.mode !== "reset-on-change" ||
      motion.changeKey != null
    )
      return;
  ...
  }, [motion.enabled, motion.mode, motion.changeKey, ...deps]);

  if (!motion.enabled || motion.mode === "none") return "static";
  if (motion.mode === "reset-on-change" && motion.changeKey != null)
    return `change-${motion.changeKey}`;
...
function motionGroupProps(motion: ResolvedMotion, renderKey: string) {
  ...
  return {
    ...
    style: isAnimated
      ? ({
          "--gen-chart-motion-duration": `${motion.durationMs}ms`,
          "--gen-chart-motion-easing": motionEase(motion.easing),
        } as React.CSSProperties)
      : undefined,
  };
}

function barMotionClassName(
  orientation: "vertical" | "horizontal",
  value: number,
) {
  if (orientation === "horizontal") {
    return value < 0
      ? "gen-chart-motion-bar gen-chart-motion-bar-horizontal-negative"
      : "gen-chart-motion-bar gen-chart-motion-bar-horizontal-positive";
  }
  return value < 0
    ? "gen-chart-motion-bar gen-chart-motion-bar-negative"
    : "gen-chart-motion-bar gen-chart-motion-bar-positive";
}
...
function renderTooltip<T>(
  ...
  const value =
    tooltipOptions.config?.valueFormatter?.(tooltip.context) ??
    (tooltip.context.value == null
      ? "-"
      : new Intl.NumberFormat("ko-KR").format(tooltip.context.value));
...
function CartesianRenderer<T>(props: CartesianChartProps<T>) {
  ...
  const motion = resolveMotion(props.motion, tokens);
  const motionRenderKey = useMotionRenderKey(motion, [
    props.data,
    props.series,
  ]);
  ...
  const padding = {
    top:
      basePadding.top +
      (legend.enabled && legend.position === "top" ? LEGEND_BAND : 0),
    right: basePadding.right,
    bottom:
      basePadding.bottom +
      (legend.enabled && legend.position === "bottom" ? LEGEND_BAND : 0),
  ...
  props.series.forEach((series) => {
    ...
    props.data.forEach((datum, index) => {
      ...
      if (typeof value === "number" && Number.isFinite(value))
        pushYAxisValue(axisId, value);
    });
  });
  if (stackedBarSeries.length > 0) {
    const stackTotals = new Map<
      string,
      { pos: number; neg: number; axisId: string }
    >();
  ...
  const resolveYAxisOptions = (axisId: string) =>
    axisId === defaultYAxisId ? props.yAxis : props.yAxes?.[axisId];
  const yScaleByAxis = new Map<
    string,
  ...
  allAxisIds.forEach((axisId) => {
    ...
    const minY =
      typeof axisOptions?.min === "number" ? axisOptions.min : minYFromData;
    const maxYCandidate =
      typeof axisOptions?.max === "number" ? axisOptions.max : maxYFromData;
    ...
    props.series.forEach((series) => {
      ...
      props.data.forEach((datum, index) => {
        ...
        const isHorizontalBar =
          series.type === "bar" && barOrientation === "horizontal";
        points.push({
          datum,
          index,
  ...
  const resolveLabelPlacement = (
    ...
    if (!props.avoidValueLabelOverlap)
      return { x: clampedX, y: baseY, hidden: false };
    ...
    for (const offset of offsets) {
      ...
      if (opts?.label) {
        ...
        const estimatedWidth = Math.max(
          12,
          opts.label.length * fontSize * 0.58,
        );
        ...
        const left =
          textAnchor === "middle"
            ? clampedX - estimatedWidth / 2
            : textAnchor === "end"
              ? clampedX - estimatedWidth
              : clampedX;
        const top =
        ...
        for (const point of points) {
          ...
          if (series.type === "bar" && series.stackId) {
            ...
            if (barOrientation === "horizontal") {
              ...
              const seriesYScale =
                yScaleByAxis.get(getSeriesYAxisId(series)) ?? yScale;
  ...
  return (
    ...
    >
      {legend.enabled && legend.position === "top" ? (
        <div
          className="gen-chart-legend"
          style={{ justifyContent: legendJustify(legend.align) }}
        >
            ...
            {(() => {
              ...
              return props.series.map((series) => {
                ...
                const seriesStrokeWidth =
                  series.strokeWidth ?? tokens.border.seriesStrokeWidth;
                const lineStrokeColor = series.strokeColor ?? color;
                const barStrokeColor =
                  series.strokeColor ??
                  tokens.border.seriesStrokeColor ??
                  color;
                ...
                const resolveBarFill = (point: Point<T>) => {
                  ...
                  if (typeof series.color === "function")
                    return series.color(point.value, point.datum, point.index);
                  return color;
                };
                const getValueLabel = (point: Point<T>) =>
                  series.valueLabelFormatter?.(
                    point.value,
                    point.datum,
                    point.index,
                  ) ?? new Intl.NumberFormat("ko-KR").format(point.value);
                const shouldShowValueLabel = (point: Point<T>) =>
                  Boolean(series.showValueLabel) &&
                  !(series.hideZeroValueLabel && point.value === 0);
                ...
                if (series.type === "line") {
                  return (
                    <React.Fragment key={series.id}>
                      <LinePath<Point<T>>
                        className="gen-chart-motion-line"
                        data={points}
                        x={(d: Point<T>) => d.x}
                        y={(d: Point<T>) => d.y}
                ...
                if (series.type === "area") {
                  return (
                    <React.Fragment key={series.id}>
                      <AreaClosed<Point<T>>
                        className="gen-chart-motion-area"
                        data={points}
                        x={(d: Point<T>) => d.x}
                        y={(d: Point<T>) => d.y}
                        yScale={yScale}
          ...
          {props.xAxis?.show !== false ? (
            ...
            props.xAxis?.position === "top" ? (
              ...
              <AxisBottom
                ...
                hideTicks={
                  barOrientation === "horizontal"
                    ? props.yAxis?.showTicks === false
                    : props.xAxis?.showTicks === false
                }
                stroke={tokens.color.axis}
                tickStroke={tokens.color.axis}
                tickLabelProps={() => ({
                ...
                numTicks={
                  barOrientation === "horizontal"
                    ? (props.yAxis?.tickCount ?? 5)
                    : props.xAxis?.tickCount
                }
                tickFormat={
                  (barOrientation === "horizontal"
                    ? props.yAxis?.tickFormat
                    : props.xAxis?.tickFormat) as
                    | ((value: any) => string)
                    | undefined
                }
          ...
          {resolveYAxisOptions(defaultYAxisId)?.show !== false ? (
            <AxisLeft
              ...
              hideTicks={
                barOrientation === "horizontal"
                  ? props.xAxis?.showTicks === false
                  : resolveYAxisOptions(defaultYAxisId)?.showTicks === false
              }
              stroke={tokens.color.axis}
              tickStroke={tokens.color.axis}
              tickLabelProps={() => ({
              ...
              numTicks={
                barOrientation === "horizontal"
                  ? undefined
                  : (resolveYAxisOptions(defaultYAxisId)?.tickCount ?? 5)
              }
              tickFormat={
                (barOrientation === "horizontal"
                  ? props.xAxis?.tickFormat
                  : resolveYAxisOptions(defaultYAxisId)?.tickFormat) as
                  | ((value: any) => string)
                  | undefined
              }
            />
            ...
            ? rightAxisEntries
                ...
                .map(([axisId, axis]) => (
                  <AxisRight
                    ...
                    tickFormat={
                      axis.tickFormat as ((value: any) => string) | undefined
                    }
                  />
                ))
            : null}
...
function PieDonutRenderer<T>(props: PieDonutChartProps<T>) {
  ...
  const legend = resolveLegend(props.legend);
  const motion = resolveMotion(props.motion, tokens);
  const motionRenderKey = useMotionRenderKey(motion, [props.data]);
  const tooltipOptions = resolveTooltip(props.tooltip);
  const [tooltip, setTooltip] = React.useState<TooltipState<T> | null>(null);
  const plotHeight = Math.max(
    0,
    props.height - (legend.enabled ? LEGEND_BAND : 0),
  );
  const radius = Math.max(8, Math.min(props.width, plotHeight) / 2 - 14);
  const outerRadius = props.outerRadius ?? radius;
  const innerRadius =
    props.kind === "donut"
      ? (props.innerRadius ?? Math.round(outerRadius * 0.58))
      : 0;
        ...
        >
          {props.data.map((datum, index) => (
            <span key={index} className="gen-chart-legend-item">
              <i
                style={{
                  background: props.color?.(datum, index) ?? colorScale(index),
                }}
              />
        ...
        <Group left={cx} top={cy}>
          <g {...motionGroupProps(motion, motionRenderKey)}>
            <Pie<T>
              data={props.data}
              pieValue={(datum: T) =>
                Math.max(0, props.value(datum, props.data.indexOf(datum)))
              }
            ...
            >
              {(pie: any) =>
                pie.arcs.map((arc: any, index: number) => {
                  const fill =
                    props.color?.(arc.data as T, index) ?? colorScale(index);
                  const [lx, ly] = pie.path.centroid(arc);
                  return (
                    <g key={`${props.kind}-${index}`}>
                      <path
                        className="gen-chart-motion-slice"
                        d={pie.path(arc) ?? ""}
                        fill={fill}
                        stroke="#fff"
                        strokeWidth={1}
...
function TreemapRenderer(props: TreemapChartProps) {
  ...
  const [tooltip, setTooltip] =
    React.useState<TooltipState<TreemapNode> | null>(null);
  ...
  const root = React.useMemo(
    () =>
      hierarchy<TreemapNode>(props.data).sum((node: TreemapNode) =>
        Math.max(0, valueAccessor(node)),
      ),
          ...
          {(tree: any) =>
            tree
              ...
              .map((node: any, index: number) => {
                ...
                const fill =
                  props.color?.(node.data as TreemapNode, node.depth, index) ??
                  colorScale(node.depth % 6);
                return (
                  <g
                    key={`${motionRenderKey}-${(node.data as TreemapNode).id}`}
                    className={
                      motion.enabled && motion.mode !== "none"
                        ? "gen-chart-motion-enter"
                        : undefined
                    }
                    style={
                      motion.enabled && motion.mode !== "none"
                        ? ({
                            "--gen-chart-motion-duration": `${motion.durationMs}ms`,
                            "--gen-chart-motion-easing": motionEase(
                              motion.easing,
                            ),
                          } as React.CSSProperties)
                        : undefined
                  ...
                  >
                    <rect
                      className="gen-chart-motion-treemap"
                      width={width}
                      height={height}
                      fill={fill}
                      stroke="#fff"
                      onClick={() =>
                        props.onNodeClick?.(node.data as TreemapNode)
                      }
                      onMouseMove={(event) => {
                        if (!tooltipOptions.enabled) return;
                        const rect =
