<!-- packages/gen-datagrid/docs/cell-edit-api.md
Documents the GenDataGrid cell editing API surface and implementation status.
-->

# GenDataGrid Cell Edit API

이 문서는 Cell Edit 관련 public API, column meta, editor context, 현재 구현 상태, 보류 항목을 정리한다. 용어 기준은 `docs/terminology.md`를 따른다.
...
- 아직 확정되지 않은 data mutation, dirty state, paste, blur commit 정책은 deferred로 남긴다.
...
| `editOnActiveCell?: boolean` | deferred | cell이 active가 되는 시점에 edit mode로 진입할지 결정하는 정책. |
| `keepEditingOnNavigate?: boolean` | deferred | navigation 중 edit mode를 유지할지 결정하는 정책. |
...
`renderEditor`와 `editorFactory`는 다음 context를 받는다.
...
| `editType` | `editorFactory`에 전달되는 resolved edit type. |
| `editOptions` | `editorFactory`에 전달되는 select options. |
| `placeholder` | `editorFactory`에 전달되는 placeholder. |
| `selectOnFocus` | `editorFactory`에 전달되는 select-on-focus 정책. |
...
- Enter: commit
- Escape: cancel
- 다른 cell activate: cancel
- custom editor에서 `commit`, `cancel`, `applyValue` 호출

## 10. 보류 API와 정책

다음 항목은 public surface 또는 계획에는 존재하지만 runtime 정책이 확정되지 않아 deferred로 남긴다.
...
| Blur commit | select dropdown, outside click, grid-in-grid boundary와 충돌할 수 있다. |
| Tab/Shift+Tab editable-cell navigation | editable cell list 계산, commit/cancel 순서, focus 이동 정책이 필요하다. |
...
- deferred API는 문서에 명시적으로 남겨 public API 오해를 줄인다.
