...
Gate 4.1-b/c/d is implemented. Treat the following API as current runtime
contract, even if older sections in this document still describe it as a
follow-up:

- `editPolicy?: GenDataGridEditPolicy` is implemented on grid props.
- `meta.editPolicy?: GenDataGridEditPolicy` is implemented as a column-level override.
- `meta.editBlurOwnership?: 'inline' | 'portal' | 'modal'` is implemented and
  takes priority over `meta.editPolicy.blurOwnership`.
- `editPolicy.startTriggers` gates reclick, double-click, Enter, F2, and
  printable-key edit entry.
- `editPolicy.continueTriggers` gates click, Tab, and Arrow continuation.
- `editPolicy.openOnEditStart` is passed to built-in and custom editors.
- Built-in editor keyboard ownership is implemented in
  `src/features/editing/builtinEditorKeyboard.ts`.
- Blur ownership and editor surface guards are implemented in
  `src/features/editing/blurPolicy.ts`.
- `GenDataGridEditorContext` includes `arrowNavigate`, `openOnEditStart`,
  `editEntryReason`, `blurOwnership`, `registerEditorSurface`,
  `unregisterEditorSurface`, `getGridRoot`, and `getEditorSurfaces`.

## Gate 4.2 Current Status

Gate 4.2 paste application is implemented for plain text clipboard data:

- `Ctrl/Cmd+V` and browser paste events use `clipboardData.getData('text/plain')`.
- Paste starts at the active cell and maps text matrix cells to visible
  row/column positions.
- Interactive editor paste is not intercepted.
- Accepted cells emit `onCellValueChange`; GenDataGrid still does not mutate
  `data` internally.
- Read-only, non-editable, and out-of-bounds cells are skipped by default.
- `pasteOptions.errorMode: 'report'` exposes paste errors through
  `pasteOptions.onError`.
- `pasteOptions.failureBehavior: 'cancelPaste'` cancels all accepted changes if
  any paste error is collected.
- Rich `text/html` paste, style paste, row creation, and type coercion remain
  deferred.
...
- grid 내부 data mutation은 하지 않으며, dirty/paste 결과는 `onCellValueChange`와 `enableDirtyState`로 소비자가 처리한다.
...
| `editCommitOnBlur?: boolean` | implemented | 기본 editor가 blur될 때 commit할지 결정한다. 기본값은 `true`. `false`를 명시하면 blur/다른 셀 activate 시 cancel한다. |
...
Gate 4.1 implements editor-opening policy for surfaces that should open immediately after edit entry, such as dropdown, datepicker, popover, and modal editors. Built-in `select` and `date` attempt native open-on-start; production-grade popups should use a custom editor.
...
Gate 4.1 extends the effective editor policy surface so built-in and custom editors can react to:

- edit entry reason
- open-on-edit-start
- keep-editing navigation
- inline vs portal vs modal blur ownership
...
- blur: 기본적으로 commit. `editCommitOnBlur={false}`이면 cancel
- 다른 cell activate: 기본적으로 commit 후 이동. `editCommitOnBlur={false}`이면 cancel 후 이동
- Escape: cancel
